...
import ChartSubsidiaryInvest from './chart/barChartSubsidiaryInvest';
import ChartSubsidiaryNonInvest from './chart/barChartSubsidiaryNonInvest';
...
// import ChartWithDropdownHolding21 from './chart/barChartHolding2 1';

export default function ReportChartNonInvest({ company }) {
  ...
  const handleDownloadAll = () => {
    ...
    filesDownloadAll.forEach((fileName) => {
      const fileUrl = `/files/vintage/results/calculator/${fileName}`;
          ...
          if (filesProcessed === filesDownloadAll.length) {
            zip.generateAsync({ type: 'blob' })
              .then((content) => {
                ...
                link.download = 'Result Summary ECL.zip';
                link.click();
              })
              .catch(err => {
        ...
        <div className="flex-grow-1 pt-3 pl-1">
          <div style={{ display: "flex", justifyContent: "center", width: "100%", minHeight: "100vh", backgroundColor: "#fbfbfb" }}>
            <div className="card flex" style={{ width: "100%", minHeight: "90%", alignSelf: "start", alignItems: "center", zIndex: "999" }}>
              <div className="card-body col-12 " style={{ justifyContent: "flex-start", alignItems: "start", height: 'auto' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                  <div style={{ display: 'flex', justifyContent: 'flex-start' }}>
                    <h2 className="mb-4 d-flex align-items-center">
                      Non-Investment ECL: Calculation Summary
                    </h2>
                  </div>

                  <div style={{ display: 'flex', justifyContent: 'flex-end' }}>

                    {/* <button className="btn btn3" onClick={handleDownloadAll} style={{ height: '40px', marginRight: '5px' }}>
                      View Result CSV
                    </button> */}
                    <button className="btn btn4" onClick={handleDownloadAll} style={{ height: '40px', marginRight: '5px' }}>
                      Download All
                    </button>
                    <button className="btn btn-outline-info" onClick={handleBack} style={{ height: '40px' }}>
                      {`<`} Back
                    </button>
                  </div>
                </div>
                ...
                {company === "" ?
                  <ChartSubsidiaryNonInvest acName={'Bahana Artha Ventura'} /> :
                  company === 'Central Holdings' ?
                  <ChartSubsidiaryNonInvest acName={'Gamma Capital'} /> :
                  <ChartSubsidiaryNonInvest acName={company} />
                }
                {/* <ChartWithDropdownHolding21/> */}
