...
export default function ModelListLRVintageHolding() {
  ...
  const [projects, setProjects] = useState([
    { id: 1, projectName: "Tariff01_Feb2025", status: 'Completed', progress: 0, reportingDate: new Date(2025, 0, 31) , type: types[0] },
    { id: 2, projectName: "Tariff02_Feb2025", status: 'Completed', progress: 0, reportingDate: new Date(2025, 11, 31) , type: types[0] },
  ]);
  ...
  const handleSave = () => {
    ...
    if (isEdit) {
      setProjects((prevProjects) =>
        prevProjects.map((p) =>
          p.id === project.id ? { ...p, projectName, reportingDate, type: projectType } : p
        )
      );
    } else {
      const newProject = {
  ...
  const handleViewCSV = (csvFileName) => {
    const fileUrl = `/files/vasicek/results/model/${csvFileName}`; 
            ...
            >
              ...
              >
                <div className="d-flex justify-content-between align-items-center p-4">
                  <h2 className="m-0">Tariff Calculation Project List</h2>
                  <Button
