...
export default function EclLossRateSekuritas() {
  ...
  const [rows, setRows] = useState([
    ...
    { id: 1, process: 'Data Control Validation', status: 'Not Started', progress: 0, rejectionComment: "" },
    { id: 2, process: 'Data Format Validation', status: 'Not Started', progress: 0, rejectionComment: "" },
    ...
    { id: 3.1, process: 'Collateral After Haircut Calculation', status: 'Not Started', progress: 0, isSubprocess: true, fileNames: [], rejectionComment: "" },
    { id: 3.2, process: 'Final ECL Calculation', status: 'Not Started', progress: 0, isSubprocess: true, fileNames: ["securities_12_31_2024.csv","journal_securities_12_31_2024"], rejectionComment: "" },
  ...
  const handleDownload = (fileNames) => {
    fileNames.forEach((fileName) => {
      const fileUrl = `/files/securities/results/calculator/${fileName}`; // Path to the file in the public folder
  ...
  const filesList = [
    'securities_calc_datatransfer.csv'
  ];
  ...
  const handleDownloadModelDataTransfer = () => {
    ...
    filesList.forEach((fileName) => {
      const fileUrl = `/files/securities/reports_securities/calculator/${fileName}`;
          ...
          if (filesProcessed === filesList.length) {
            zip.generateAsync({ type: 'blob' })
              .then((content) => {
                ...
                link.download = 'Data Transfer Checking Securities Calculator.zip';
                link.click();
              })
              .catch(err => {
  ...
  const filesListQuality = [
    'receivables_dataquality.csv',
    'haircut_dataquality.csv'
  ];
          ...
          if (filesProcessed === filesListQuality.length) {
            zip.generateAsync({ type: 'blob' })
              .then((content) => {
                const link = document.createElement('a');
                link.href = URL.createObjectURL(content);
                link.download = 'Data Quality Checking Securities Calculator.zip';
                link.click();
              })
              .catch(err => {
                console.error('Error generating zip:', err);
  ...
  const executionTimeMapping = {
    "Collateral After Haircut Calculation": 4 * 1000,
              ...
              <div className="card-body col-12 d-flex flex-column" style={{ justifyContent: "flex-start", height: 'auto' }}>
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                  <div style={{ display: 'flex', justifyContent: 'flex-start' }}>
                    <h2 className="m-4">Non-Investment ECL - Securities Asset</h2>
                  </div>
