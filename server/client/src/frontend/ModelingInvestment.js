...
export default function ModelingInvestment() {
    ...
    const [rows, setRows] = useState([
        ...
        { id: 3.01, process: "Z-score calculation", status: "Not Started", progress: 0, isSubprocess: true, fileNames: ["zscore.csv"], rejectionComment: "" },
        { id: 3.02, process: "Asset Correlation Calculation", status: "Not Started", progress: 0, isSubprocess: true, fileNames: ["rating_pd.csv"], rejectionComment: "" },
        { id: 3.03, process: "Vasicek FPD Calculation", status: "Not Started", progress: 0, isSubprocess: true, fileNames: ["vasicek_fpd.csv"], rejectionComment: "" },
        { id: 3.04, process: 'Vasicek CPD Calculation', status: 'Not Started', progress: 0, isSubprocess: true, fileNames: ["vasicek_cpd.csv"], rejectionComment: "" },
        { id: 3.05, process: 'Vasicek MPD Calculation', status: 'Not Started', progress: 0, isSubprocess: true, fileNames: ["vasicek_mpd.csv"], rejectionComment: "" },
        { id: 3.06, process: 'Monthly FPD Calculation', status: 'Not Started', progress: 0, isSubprocess: true, fileNames: ["monthly_fpd.csv"], rejectionComment: "" },
        { id: 3.07, process: 'Monthly MPD Calculation', status: 'Not Started', progress: 0, isSubprocess: true, fileNames: ["monthly_mpd.csv"], rejectionComment: "" },
        { id: 3.08, process: 'Monthly CPD Calculation', status: 'Not Started', progress: 0, isSubprocess: true, fileNames: ["monthly_cpd.csv"], rejectionComment: "" },
        { id: 3.09, process: 'Weighted CPD Calculation', status: 'Not Started', progress: 0, isSubprocess: true, fileNames: ["weighted_cpd.csv"], rejectionComment: "" },
        { id: 3.10, process: 'LGD Calculation', status: 'Not Started', progress: 0, isSubprocess: true, fileNames: ["lgd.csv"], rejectionComment: "" },
    ]);


    const handleDownload = (fileNames) => {
        fileNames.forEach((fileName) => {
            const fileUrl = `/files/vasicek/process/${fileName}`;
    ...
    const filesList = [
        'vasicek_model_datatransfer.csv'
    ];
    ...
    const handleDownloadModelDataTransfer = () => {
        ...
        filesList.forEach((fileName) => {
            const fileUrl = `/files/vasicek/reports_vasicek/model/${fileName}`;
                    ...
                    if (filesProcessed === filesList.length) {
                        zip.generateAsync({ type: 'blob' })
                            .then((content) => {
                                ...
                                link.download = 'Data Transfer Checking Invest Model.zip';
                                link.click();
                            })
                            .catch(err => {
    ...
    const filesListQuality = [
        ...
        'MEV_Scenario_dataquality.csv',
        'MEV_Historical_dataquality.csv',
        'PDTTC_dataquality.csv',
        'Weighting_dataquality.csv',
    ];
                    ...
                    if (filesProcessed === filesListQuality.length) {
                        zip.generateAsync({ type: 'blob' })
                            .then((content) => {
                                const link = document.createElement('a');
                                link.href = URL.createObjectURL(content);
                                link.download = 'Data Quality Checking Invest Model.zip';
                                link.click();
                            })
                            .catch(err => {
                                console.error('Error generating zip:', err);
                    ...
                    if (filesProcessed === filesDownloadAll.length) {
                        zip.generateAsync({ type: 'blob' })
                            .then((content) => {
                                const link = document.createElement('a');
                                link.href = URL.createObjectURL(content);
                                link.download = 'Investment Tariff.zip';
                                link.click();
                            })
                            .catch(err => {
                                console.error('Error generating zip:', err);
    ...
    const executionTimeMapping = {
        "Z-score Calculation": 3 * 1000,
        "Asset Correlation Calculation": 2 * 1000,
        "Vasicek FPD Calculation": 8 * 1000,
        "Vasicek CPD Calculation": 5 * 1000,
        "Vasicek MPD Calculation": 3 * 1000,
        "Monthly FPD Calculation": 5 * 1000,
        "Monthly MPD Calculation": 8 * 1000,
        "Monthly CPD Calculation": 10 * 1000,
        "Weighted CPD Calculation": 8 * 1000,
        "LGD Calculation', status": 5 * 1000,
    };
                            ...
                            <div className="card-body col-12 d-flex flex-column" style={{ justifyContent: "flex-start", height: 'auto' }}>
                                ...
                                <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                                    <div style={{ display: 'flex', justifyContent: 'flex-start' }}>
                                        <h2 className="m-4">Investment Tariff - Investment Portfolio</h2>
                                    </div>
