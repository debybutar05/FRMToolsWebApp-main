...
export default function ModelListLRVintageHolding() {
  const types = [
  ...
  const companyOptions = [
    ...
    { value: 'Gamma Capital', label: 'Gamma Capital' },
    { value: 'SafeGuard Insurance', label: 'SafeGuard Insurance' },
    { value: 'SafeGuard Partners', label: 'SafeGuard Partners' },
    { value: 'TrustInsure', label: 'TrustInsure' },
    { value: 'National Reinsurance', label: 'National Reinsurance' },
    { value: 'SecureCover', label: 'SecureCover' },
    { value: 'Insurance Subsidiary', label: 'Insurance Subsidiary' },
    { value: 'HealthTrust', label: 'HealthTrust' },
    { value: 'SureGuarantee', label: 'SureGuarantee' },
  ];
  ...
  const companyLabel = [
    { value: 'Central Holdings', label: 'CH' },
    { value: 'Investment Subsidiary', label: 'INVEST' },
    { value: 'Alpha Asset Management', label: 'AAM' },
    { value: 'Beta Securities', label: 'BSEC' },
    { value: 'Gamma Capital', label: 'GCAP' },
    { value: 'SafeGuard Insurance', label: 'SGI' },
    { value: 'SafeGuard Partners', label: 'SGP' },
    { value: 'TrustInsure', label: 'TRUST' },
    { value: 'National Reinsurance', label: 'NRE' },
    { value: 'SecureCover', label: 'SCOV' },
    { value: 'Insurance Subsidiary', label: 'INSUB' },
    { value: 'HealthTrust', label: 'HT' },
    { value: 'SureGuarantee', label: 'SGUAR' },
  ];


  const [projects, setProjects] = useState([
    { id: 1, projectName: "Tariff_NonInvest_BSEC", name: 'Beta Securities', status: 'In Progress', progress: 0, reportingDate: new Date(), type: types[0] },
    { id: 2, projectName: "Tariff_NonInvest_INVEST", name: 'Investment Subsidiary', status: 'In Progress', progress: 0, reportingDate: new Date(), type: types[1] },
    { id: 3, projectName: "Tariff_NonInvest_SGI", name: 'SafeGuard Insurance', status: 'Completed', progress: 0, reportingDate: new Date(), type: types[1] },
  ]);
  ...
  useEffect(() => {
    if (!modalData.isEdit && modalData.companyName) {
      ...
      const newProjectName = `Tariff_NonInvest_${companyAbbreviation}_${formattedDate}`;
  ...
  const handleDownloadModel = (project) => {
    ...
    if (project.type.label === types[0].label) {
      filesList = [
        'lrf.csv',
      ];
    } else if (project.type.label === types[1].label) {
      filesList = [
        'ict.csv',
    ...
    if (filesList.length > 0) {
      filesList.forEach((fileName) => {
        const fileUrl =
          project.type.label === types[1].label
            ? `/files/vintage/results/model/${fileName}`
            : project.type.label === types[0].label
              ? `/files/securities/results/model/${fileName}`
              : `/files/securities/results/model/${fileName}`;
      ...
      console.log('No files to download for this project type');
    }
  };
  ...
  const columns = [
    {
      ...
      selector: (project) => (
        project.status === 'Completed' ?
          ...
          <Link
            to={{
              pathname: project.type.label === types[0].label
                ? "/modeling-loss-rate-vintage"
                : project.type.label === types[1].label
                  ? "/modeling-loss-rate-sekuritas"
                  : "/modeling-loss-rate-sekuritas", // Optional: Handle other cases if needed
              state: project.status !== "Completed"
                ? { projectName: project.projectName, reportingDate: project.reportingDate }
                : {},
            }}
            style={{
              textDecoration: "underline",
            ...
            >
              ...
              >
                <div className="d-flex justify-content-between align-items-center p-4">
                  <h2 className="m-0">Non-Investment Tariff</h2>
                  <Button
...
// import 'bootstrap/dist/css/bootstrap.min.css';
// import DataTable from 'react-data-table-component';
// import { Pencil, Trash, HouseFill, Download } from 'react-bootstrap-icons';
// import Modal from 'react-bootstrap/Modal';
// import Button from 'react-bootstrap/Button';
// import Select from 'react-select';
// import Swal from 'sweetalert2';
...
// export default function ModelListLRVintageHolding() {
//   const types = [
//     { value: 'Receivables Portfolio Tariff', label: 'Modeling Investment' },
...
//     { value: 'BAHANA – Kapital Investa', label: 'BAHANA – Kapital Investa' },
//     { value: 'GRAHANIAGA TATAUTAMA', label: 'GRAHANIAGA TATAUTAMA' },
//     { value: 'ASKRINDO - Insurance', label: 'ASKRINDO - Insurance' },
//     { value: 'Asuransi JASINDO', label: 'Asuransi JASINDO' },
//     { value: 'JASA RAHARJA', label: 'JASA RAHARJA' },
//     { value: 'Central Holdings - Life', label: 'Central Holdings - Life' },
//     { value: 'Mandiri HealthTrust', label: 'Mandiri HealthTrust' },
//   ];
...
//   const [projects, setProjects] = useState([
//     { id: 1, name: 'GRAHANIAGA TATAUTAMA', status: 'In Progress', progress: 0, reportingDate: "10/31/2024", type: types[0] },
//     { id: 2, name: 'ASKRINDO - Insurance', status: 'Completed', progress: 0, reportingDate: "9/30/2023", type: types[0] },
//     { id: 3, name: 'JASA RAHARJA', status: 'Completed', progress: 0, reportingDate: "10/31/2022", type: types[0] },
//   ]);
...
//               >
//                 <div className="d-flex justify-content-between align-items-center p-4">
//                   <h2 className="m-0">List of Projects for Model Receivables Portfolio</h2>
//                   {/* <Button
//                     className="btn btn-primary"
//                     onClick={() => openModal(false)}
//                     style={{ backgroundColor: '#00338D', color: 'white', border: 'None' }}
