...
export default function EclInvestment() {
  ...
  const [rows, setRows] = useState([
    ...
    // { id: 1, process: 'Data Validation', status: 'Not Started', progress: 0, error: '', isDropdown: true, isSubprocess: false },
    { id: 1, process: 'Data Transfer Checking', status: 'Not Started', progress: 0, fileNames: [], rejectionComment: "" },
    { id: 2, process: 'Data Quality Checking', status: 'Not Started', progress: 0, fileNames: [], rejectionComment: "" },
    // { id: 2, process: 'Generate Data Validation Result', status: 'Not Started', error: '' },
    { id: 3, process: 'Calculate ECL', status: 'Not Started', progress: 0, error: '', isDropdown: true, isSubprocess: false, fileNames: [], rejectionComment: "" },
    { id: 3.1, process: 'Discount Rate Calculation', status: 'Not Started', progress: 0, isSubprocess: true, fileNames: [], rejectionComment: "" },
    { id: 3.2, process: 'Final ECL Calculation', status: 'Not Started', progress: 0, isSubprocess: true, fileNames: ["investment_12_31_2024.csv","journal_vasicek_12_31_2024.csv"], rejectionComment: "" },
    // { id: 4, process: 'Generate ECL Calculation Result', status: 'Not Started', error: '' },
  ]);

  const handleDownload = (fileNames) => {
    fileNames.forEach((fileName) => {
      const fileUrl = `/files/vasicek/results/calculator/${fileName}`; // Path to the file in the public folder
  ...
  const handleDownloadModelDataTransfer = () => {
    ...
    filesList.forEach((fileName) => {
      const fileUrl = `/files/vasicek/reports_vasicek/calculator/${fileName}`;
          ...
          if (filesProcessed === filesList.length) {
            zip.generateAsync({ type: 'blob' })
              .then((content) => {
                ...
                link.download = 'Data Transfer Checking Invest Calculator.zip';
                link.click();
              })
              .catch(err => {
  ...
  const filesListQuality = [
    'active_portfolio_dataquality.csv',
  ];
          ...
          if (filesProcessed === filesListQuality.length) {
            zip.generateAsync({ type: 'blob' })
              .then((content) => {
                const link = document.createElement('a');
                link.href = URL.createObjectURL(content);
                link.download = 'Data Quality Checking Invest Calculator.zip';
                link.click();
              })
              .catch(err => {
                console.error('Error generating zip:', err);
  ...
  const executionTimeMapping = {
    "Discount Rate Calculation": 4 * 1000,
              ...
              <div className="card-body col-12 d-flex flex-column" style={{ justifyContent: "flex-start", height: 'auto' }}>
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                  <div style={{ display: 'flex', justifyContent: 'flex-start' }}>
                    <h2 className="m-4">Investment ECL - Investment Portfolio</h2>
                  </div>
                  ...
                  <div style={{ display: 'flex', justifyContent: 'flex-end' }}>
                    ...
                    {(allApproved && allCompleted) && (
                      <button className="btn btn-primary" style={{ height: '40px', marginLeft: '5px' }}
                        onClick={() => navigateReport('/Report-Investment')}
                      >
                        View Report
                      </button>
